...
export class MerchantClient {
  constructor(readonly profile: MerchantProfile) {}

  initializeInstruction(): TransactionInstruction {
    return initializeMerchantInstruction(
      this.profile.authority,
      this.profile.treasury,
      this.profile.feeTreasury,
      this.profile.feeBps,
    );
  }

  createPaymentInstruction(intent: MerchantPaymentIntent, payer: PublicKey) {
    if (intent.mint && intent.tokenProgram) {
      throw new Error(
        "Use createTokenPayment() and provide payer/escrow token accounts for token payments.",
      );
    }
